...
export async function GET(request: Request) {
    console.log("--- API GET /api/admin/fees (Prisma) START ---");
    ...
    try {
        ...
        console.error("--- API GET /api/admin/fees (Prisma) FAILED ---", error);
...
export async function POST(req: Request) {
    console.log("--- API POST /api/admin/fees (Prisma Create FeeRule) START ---");
    const session = await getServerSession(authOptions);
    if (!session?.user?.id || (session.user as any).role !== 'ADMIN') {
        return NextResponse.json({ message: 'Forbidden: Admin access required' }, { status: 403 });
    }
    ...
    try {
        ...
        console.error("--- API POST /api/admin/fees (Prisma Create FeeRule) FAILED ---", error);
...
// For updating individual rules, use a dynamic route like /api/admin/fees/[ruleId]
export async function PUT(req: Request) {
    console.log("--- API PUT /api/admin/fees (Prisma Update Default Fee) START ---");
    const session = await getServerSession(authOptions);
    if (!session?.user?.id || (session.user as any).role !== 'ADMIN') {
        return NextResponse.json({ message: 'Forbidden: Admin access required' }, { status: 403 });
    }
    ...
    try {
        ...
        console.error("--- API PUT /api/admin/fees (Prisma Update Default Fee) FAILED ---", error);
