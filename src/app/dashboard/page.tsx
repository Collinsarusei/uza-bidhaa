...
function DashboardContent() {
  ...
  // const isMobile = useIsMobile(); // isMobile hook not strictly necessary for Sheet display logic
  ...
  const renderHeader = () => (
     <div className="flex justify-between items-center mb-6 border-b pb-4 h-16 px-4 md:px-6 sticky top-0 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 z-10">
        ...
        <div className="flex items-center gap-1 md:gap-2">
            ...
            <div className="hidden md:flex items-center gap-2"> 
                ...
                {isAdminUser && (
                 ...
                 {session?.user && (
                     <TooltipProvider><Tooltip><TooltipTrigger asChild>
                         <Sheet open={isProfileSheetOpen} onOpenChange={setIsProfileSheetOpen}>
                             ...
                             <SheetContent className="w-[400px] sm:w-[540px] p-0 flex flex-col"><SheetHeader className="p-6 border-b sticky top-0 bg-background z-10"><SheetTitle>My Profile</SheetTitle><SheetDescription>View and manage your profile.</SheetDescription></SheetHeader><div className="flex-1 overflow-y-auto p-6"><ProfileContent /></div></SheetContent>
                         </Sheet>
                 ...
                 {session?.user && (
                    <TooltipProvider><Tooltip><TooltipTrigger asChild>
                        {/* Notification button is the trigger for the sheet below */}
        ...
        {/* Notification Sheet - now always rendered, controlled by isNotificationsSheetOpen state */}
  ...
  const renderItemCard = (item: Item) => {
    ...
    if (item.createdAt) {
        try {
            if (typeof item.createdAt === 'string') {
                ...
                if (isValid(parsedDate)) timeSinceListed = formatDistanceToNow(parsedDate, { addSuffix: true });
                else {
    ...
    return (
        <Card key={item.id} className="flex flex-col overflow-hidden hover:shadow-lg transition-shadow duration-200 ease-in-out">
            <CardHeader className="p-0"><Link href={`/item/${item.id}`} passHref className="block relative w-full aspect-square">
            ...
            <CardFooter className="p-4 flex flex-col gap-2">
                <Link href={`/item/${item.id}`} passHref className="w-full"><Button variant="outline" className="w-full hover:bg-accent hover:text-accent-foreground transition-colors">View Details</Button></Link>
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
            <div className="container mx-auto p-4 md:p-6">
                {renderHeader()} 
                <div className="mb-4"><Input placeholder="Search items by name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="max-w-sm bg-card border-border"/></div>
                 <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">{renderLoadingSkeletons()}</div>
            </div>
        </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <div className="container mx-auto p-4 md:p-6">
        ...
        {!error && !isLoadingItems && !searchTerm && items.length === 0 && (
          ...
          {!error && filteredItems.length > 0 && (<div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">{filteredItems.map(renderItemCard)}</div>)}
      </div>
    </div>
  );
}
